...
const admin_account_model = require('../model/admin_model');
...
exports.createAdmin = async(req,res)=>{
         ...
         if(username=="" || password==""){
                res.status(402).json({error:"Please fill all spaces"})
         }else{
                try{
                       const save_Admin = await admin_account_model.create({username:username,password:password});
                       if(save_Admin){
                          res.status(200).json({message:"Admin details has been created"});

                       }else{
                           res.status(402).json({error:"database error"});
                       }
                }catch(err){
...
exports.AdminLogin = async(req,res)=>{
         const {username,password}=req.body;
         const getAdmin = await admin_account_model.findOne({username:username,password:password});
         if(!getAdmin){
           
             res.json({error:"incorrect username/password",status:403})
         }else{
                const token = jwt.sign({id:getAdmin._id},process.env.ACCESS_TOKEN_SECRET,{expiresIn:"1h"});
                 res.cookie("jwtadmin",token,{httpOnly:true,maxAge:36000000});
                  return res.json({token:token,status:200})
         }
         ...
         const  result= await Account_Model.create({account_number:AccountNumber,account_name:AccountName,password:hashedpassword})
          try{
            if(result){
               ...
               if(save_balance){
                     res.json({success:"Account has been Created",data:result,status:200})
               }else{
                    res.json({error:"error saving balance"})
               }
              
              }else{
                res.json({error:"Unable to create accoount"})
              }
...
exports.search_balance = async(req,res)=>{
...
exports.updateBalance = async(req,res)=>{
      ...
      if(accountNumber=="" || Balance == "" || Income=="" || Expenses ==""){
        ...
        try{
            ...
            if(get_balance){
                ...
                if(saveupdate){
                    res.json({success:"Account has been updated",status:200});
                }else{
 ...
 exports.uploadBank = async(req,res)=>{ 
     ...
     const acctnum = getRandom10DigitNumber();
     ...
     if(acctnum =="" && acctname == "" && bank == ""){
        ...
        try{
            ...
            if(saveBank){
                return  res.json({success:"Bank Saved",status:200})
            }else{
